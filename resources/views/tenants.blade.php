            ...
            <table class="table-auto w-full mt-1 rounded-lg shadow-md overflow-hidden">
                ...
                <tbody class="bg-white">
                    @foreach($groupedTenants as $tenantId => $tenantRooms)
                        <tr class="border-t border-gray-300 cursor-pointer" onclick="window.location='{{ route('tenants.show', $tenantRooms->first()['tenant']->id) }}'">
                            <td class="px-4 py-2">{{ ucfirst($tenantRooms->first()['tenant']->name) }} {{ ucfirst($tenantRooms->first()['tenant']->last_name) }}</td>
